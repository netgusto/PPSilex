...
class PaasPlatformServiceProvider implements ServiceProviderInterface {
    ...
    public function register(Application $app) {
        ...
        $app['config.site'] = $app->share(function() use ($app, $parameters) {
            ...
            #die();
            $dbbackedconfig = new PulpyServices\Config\Loader\DbBackedConfigLoaderService(
                $app['orm.em'],
                $parameters
            );
        ...
        if($storage === 'S3') {
            ...
            $app['fs.persistent'] = $app->share(function() use ($app) {
                ...
                return new PulpyServices\PersistentStorage\S3PersistentStorageService(
                    $s3_bucket,
                    $s3_keyid,
                    $s3_secret,
                    $s3_httpbaseurl
        ...
        $app['post.cachehandler'] = $app->share(function() use ($app) {
            return new PulpyServices\CacheHandler\EventedPostCacheHandlerService(
                $app['fs.persistent'],
                $app['system.status'],
